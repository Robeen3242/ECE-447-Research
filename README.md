...
## Our Repo
In the root folder are our All-CNN style CIFAR-10 models `cnngeneral.py`, `gcnngeneral.py`, and `gcnn_p4mgeneral.py` as well as our report and the paper we based it off of. Our prototype/legacy code is in the `old_models` folder, which are the models we used to produce the results for our presentation. For our raw experiments, we chose to opt for the `___general.py` files instead. Their metrics and `.pth` files can be found in the `models` folder. 
