...
# python your_script.py --model all --dataset-npz ./data/rotated_mnist.npz
@dataclass
class Config:
    seeds: tuple = (42,)
    split_seed: int = 42
...
def main():
    parser = argparse.ArgumentParser(
        description="Z2 / P4 / P4M comparison on pre-rotated Rotated-MNIST"
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    parser.add_argument("--dataset-npz", type=str, default=CFG.prerotated_npz_path)
    ...
    cfg.prerotated_npz_path = args.dataset_npz

    os.makedirs(cfg.out_dir, exist_ok=True)

    print("=" * 80)
    print("Pre-rotated Rotated-MNIST Comparison")
    print("=" * 80)
    print(f"Device: {DEVICE}")
    print(f"Output: {cfg.out_dir}")
    print(f"Dataset: {cfg.prerotated_npz_path}")
