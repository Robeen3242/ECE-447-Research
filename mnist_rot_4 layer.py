...
class FixedRotatedMNIST(Dataset):
    """MNIST with a deterministic fixed rotation angle per sample."""
...
def save_comparison(cfg: Config, z2_summary, p4m_summary):
    comparison = {
        "z2": z2_summary["aggregate"],
        "p4": p4_summary["aggregate"],
        "delta_test_accuracy_mean_p4m_minus_z2": (
            p4m_summary["aggregate"]["test_accuracy_mean"] - z2_summary["aggregate"]["test_accuracy_mean"]
        ),
    }
    ...
    with open(os.path.join(cfg.out_dir, "comparison_summary.md"), "w") as f:
        f.write("# Rotated-MNIST Z2 vs P4 (Matched Protocol)\n\n")
        f.write(
            f"- Z2 test accuracy mean/std: {z2_summary['aggregate']['test_accuracy_mean']:.2f}% "
            f"+- {z2_summary['aggregate']['test_accuracy_std']:.2f}%\n"
        )
        f.write(
            f"- P4m test accuracy mean/std: {p4m_summary['aggregate']['test_accuracy_mean']:.2f}% "
            f"+- {p4m_summary['aggregate']['test_accuracy_std']:.2f}%\n"
        )
        f.write(
            f"- Delta (P4m - Z2): {comparison['delta_test_accuracy_mean_p4m_minus_z2']:.2f} points\n"
        )


def run_for_model(cfg: Config, model_name: str):
    if model_name == "z2":
        builder = lambda: Z2MNISTCNN(channels=cfg.z2_channels, num_classes=cfg.num_classes)
    elif model_name == "p4":
        builder = lambda: P4MNISTCNN(widths=cfg.p4_widths, num_classes=cfg.num_classes)
    else:
        raise ValueError("model_name must be one of: z2, p4m")
...
def main():
    parser = argparse.ArgumentParser(description="Matched Z2/P4 training on a custom Rotated-MNIST dataset.")
    parser.add_argument("--model", type=str, default="both", choices=["z2", "p4", "both"])
    ...
    print("=" * 80)
    print("Rotated-MNIST Matched Comparison")
    print("=" * 80)
    print(f"Device: {DEVICE}")
    print(f"Output: {cfg.out_dir}")
    print(f"Seeds: {cfg.seeds}")

    if args.model in ("z2", "both"):
        z2_summary = run_for_model(cfg, "z2")
    else:
        z2_summary = None

    if args.model in ("p4", "both"):
        p4m_summary = run_for_model(cfg, "p4")
    else:
        p4m_summary = None

    if z2_summary is not None and p4m_summary is not None:
        save_comparison(cfg, z2_summary, p4m_summary)
